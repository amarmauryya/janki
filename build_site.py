...
footer = """
    <!-- Background Music -->
    <div class="fixed bottom-8 left-8 z-40 flex flex-col gap-2 items-start">
        <div id="music-menu" class="hidden bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border border-bloom-pink/20 rounded-2xl shadow-xl p-2 w-48 flex-col gap-1 transition-all origin-bottom-left">
            <div class="text-xs text-bloom-pink/70 font-semibold px-2 pt-1 pb-2 uppercase tracking-wider">Select Track</div>
            <button class="music-option flex items-center gap-3 p-2 rounded-xl hover:bg-bloom-pink/10 text-bloom-text transition-colors text-left" data-src="audio.mp3">
                <span class="material-symbols-outlined text-bloom-pink text-lg">play_circle</span>
                <span class="text-sm font-medium">Track 1</span>
            </button>
            <button class="music-option flex items-center gap-3 p-2 rounded-xl hover:bg-bloom-pink/10 text-bloom-text transition-colors text-left" data-src="audio2.mp3">
                <span class="material-symbols-outlined text-bloom-pink text-lg">play_circle</span>
                <span class="text-sm font-medium">Track 2</span>
            </button>
            <div class="h-[1px] bg-bloom-pink/10 my-1"></div>
            <button id="music-stop" class="flex items-center gap-3 p-2 rounded-xl hover:bg-red-50 text-red-400 transition-colors text-left">
                <span class="material-symbols-outlined text-lg">stop_circle</span>
                <span class="text-sm font-medium">Stop Music</span>
            </button>
        </div>
        <button id="music-toggle" class="w-14 h-14 rounded-full bg-white/80 dark:bg-slate-800/80 backdrop-blur shadow-lg flex items-center justify-center text-bloom-pink border border-bloom-pink/20 transition-all hover:scale-110">
            <span class="material-symbols-outlined text-2xl" id="music-icon">library_music</span>
        </button>
    </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            // Procedural Flowers Full Screen Garden
            if (typeof Flw !== 'undefined') {
                const flowerOptionsBase = {
                    widthStart: 4.0, widthEnd: 0.8,
                    colorStart: Flw.Color.createWithHex('#2b5314'), colorEnd: Flw.Color.createWithHex('#68b030'),
                    maxDeepness: 3, maxDeepnessVar: 1, maxDeepnessTwisted: 3, maxDeepnessTwistedVar: 0,
                    maxDeepnessMajor: 2, maxDeepnessMajorVar: 1, maxDeepnessMaster: 5, maxDeepnessMasterVar: 2,
                    headSize: 42, headSizeVar: 12, leafSize: 28, leafSizeVar: 8,
                    ...
                    radius: 40, radiusVar: 30, globalDirection: Math.PI / 2, growVelocity: 0.195,
                    strokeBranchWidth: 0, strokeBranchColor: '#5c5c5c', strokeLeafWidth: 0.1, strokeLeafColor: '#161616',
                    strokeHeadWidth: 0.15, strokeHeadColor: '#1b1b1b'
                };
                
                ['flower-1', 'flower-2', 'flower-3'].forEach((id, idx) => {
                    const el = document.getElementById(id);
                    if (el) {
                        const defaultW = id === 'flower-2' ? '340px' : '300px';
                        const defaultH = id === 'flower-2' ? '850px' : '750px';
                        if (!el.style.width) el.style.width = defaultW;
                        if (!el.style.height) el.style.height = defaultH;
                        
                        setTimeout(() => {
                            try {
                                Flw.LimitedFlower.create(el, flowerOptionsBase, true);
                            } catch (err) {
                                console.error('Garden flower error:', err);
                            }
                        }, idx * 400 + 200);
                    }
                });
            ...
            musicBtn.addEventListener('click', (e) => {
                ...
                musicMenu.classList.toggle('flex');
            });
            
            // Close menu when clicking outside
            document.addEventListener('click', () => {
                musicMenu.classList.add('hidden');
                musicMenu.classList.remove('flex');
            });
            ...
            musicStop.addEventListener('click', (e) => {
                e.stopPropagation();
                audio.pause();
                musicIcon.textContent = 'library_music';
...
pages['index.html'] = ("home", """
    <section class="relative min-h-[90dvh] flex items-center pb-24 overflow-hidden">
        <!-- Full Screen Background Flowers -->
        <div id="full-garden" class="absolute inset-0 pointer-events-none z-0 overflow-hidden">
            <div id="flower-1" class="absolute -bottom-10 left-[0%] md:left-[3%] pointer-events-none opacity-80" style="width: 300px; height: 750px;"></div>
            <div id="flower-2" class="absolute -bottom-10 left-[35%] md:left-[42%] pointer-events-none opacity-80" style="width: 340px; height: 850px;"></div>
            <div id="flower-3" class="absolute -bottom-10 right-[0%] md:right-[3%] pointer-events-none opacity-80" style="width: 300px; height: 750px; transform: scaleX(-1);"></div>
        </div>
